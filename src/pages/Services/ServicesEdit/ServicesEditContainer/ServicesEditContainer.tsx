...
const validationSchema = yup.object({
  ...
  Cost: yup.number().required("Costo es obligatorio"),
})

export const ServicesEditContainer = () => {
  ...
  const [service, setService] = useState<ServiceInterface>(null)
  ...
  async function editService(request: ServiceEditInterface) {
    setIsLoadingAction(true)

    const { data, status }: any = await ServicesService.editService(
      request,
      service.IdService
    )
    if (status == ConstantHttpErrors.OK) {
      setModal({
        isOpen: true,
        type: "success",
        message: ConstantServiceMessage.SERVICE_EDIT_SUCCESS,
      })
